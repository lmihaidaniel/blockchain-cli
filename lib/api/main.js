  ...
  io.on('connection', client => {
    ...
    client.on('request_certificate', data => {
      ...
      if (
        ...
        block_data.private_key = signature.generatePrivateKey()
        is_generated_private_key = true
      } else {
        block_data.private_key = data.private_key
      }

      block_data.key = signature.getPublicKey(block_data.private_key)
      ...
      blockchain.mine(block_data, certificate=>{
        ...
        // if (is_generated_private_key) {    WE SHOULD ONLY RETURN IT WHEN GENERATED
          socket_data.private_key = block_data.private_key
        // }
