...
class Blockchain {
  ...
  generateNextTransactionBlock(blockData, privateKey) {
    const assetBlock = this.findAssetBlock(blockData.asset_hash)

    if (!assetBlock) {
      logger.log('❌ invalid asset hash')
      return null
    }

    const signature = Signature.sign(blockData, privateKey, true)
    const publicKey = Buffer.from(assetBlock.data.key, 'base64')

    if (!Signature.verify(blockData, signature, publicKey)) {
  ...
  findAssetBlock(asset_hash) {
    const blocks = this.blockchain.filter(block => {
      return block.data.asset_hash === asset_hash;
    })
